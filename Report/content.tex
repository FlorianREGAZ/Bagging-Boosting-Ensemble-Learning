...
The examples indicate that at least one Boosting methods, AdaBoost or Gradient Boosting, is more accurate than Bagging and Random Forest.

% TODO: add general conclusion of the examples
% - talk about results of the examples
% - talk about what results were expected and what the examples didn't show but is also important to know

% Own results
% - 

% Popular Ensemble Methods: An Empirical Study (https://www.d.umn.edu/~rmaclin/cs5751/notes/opitz-jair99.pdf)
% - Bagging ensemble nearly always outperforms a single classifier
% - Boosting ensemble can greatly outperform both Bagging and a single classifier. However, for some data sets 
%   Boosting may show zero gain or even a decrease in performance from a single classifier (due to overfitting on noisy data)
% - much of the performance enhancement for an ensemble comes with the first few classifiers combined, but that Boosting decision
%   trees may continue to further improve with larger ensemble sizes
% - Conclusion: Bagging is appropritae for most problems, boosting when appropriate might be even better

% Comparison among Methods of Ensemble Learning (https://ieeexplore.ieee.org/abstract/document/6597704)
% -  there is no single winner here -> everything depends upon our domain and what we're going to do
